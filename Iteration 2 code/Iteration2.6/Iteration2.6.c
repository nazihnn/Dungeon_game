...
int Playerposition_middleroom(int x, int y, int width, int height) //check move of player in middle room
{
...
int Playerposition_leftroom(int x, int y, int width, int height) //check move of player in left room
{
    if (x >= width - 1 && y == height / 2) {
        return MIDDELROOM;
    } else if (x >= width - 1 && y != height / 2) {
...
int Playerposition_rightroom(int x, int y, int width, int height) {
    if (x <= 0 && y == height / 2) {
        return MIDDELROOM;
    } else if (x <= 0 && y != height / 2) {
        return KEEPSTATE;
...
int Playerposition_uproom(int x, int y, int width, int height) {
...
int Playerposition_downroom(int x, int y, int width, int height) {
    if (x == width / 2 && y == 0) {
        return MIDDELROOM;
...
people Playermovement(people mpeople, int room) //player move
{
    char ch = _getch();
    mpeople.step++;
    ...
    if (room == MIDDELROOM) {
        int n = Playerposition_middleroom(tmpx, tmpy, roomwidth[MIDDELROOM], roomheight[MIDDELROOM]);
        if (n == PLAYERMOVE) {
        ...
        int n = Playerposition_leftroom(tmpx, tmpy, roomwidth[LEFTROOM], roomheight[LEFTROOM]);
        if (n == PLAYERMOVE) {
            mdraw[mpeople.y][mpeople.x] = ' ';
            mpeople.x = tmpx;
            mpeople.y = tmpy;
        ...
        int n = Playerposition_rightroom(tmpx, tmpy, roomwidth[RIGHTROOM], roomheight[RIGHTROOM]);
        if (n == PLAYERMOVE) {
            mdraw[mpeople.y][mpeople.x] = ' ';
            mpeople.x = tmpx;
            mpeople.y = tmpy;
        ...
        int n = Playerposition_uproom(tmpx, tmpy, roomwidth[UPROOM], roomheight[UPROOM]);
        if (n == PLAYERMOVE) {
            mdraw[mpeople.y][mpeople.x] = ' ';
            mpeople.x = tmpx;
            mpeople.y = tmpy;
        ...
        int n = Playerposition_downroom(tmpx, tmpy, roomwidth[DOWNROOM], roomheight[DOWNROOM]);
        if (n == PLAYERMOVE) {
            mdraw[mpeople.y][mpeople.x] = ' ';
            mpeople.x = tmpx;
            mpeople.y = tmpy;
...
void explorer_mode() {
    ...
    for (int i = 1, j = 6; i <= 5; i++, j += 3) {
        ...
        while (flg) {
            ...
            playpeople = Playermovement(playpeople, roomid);
            for (int w = 0; w < j; w++) {
