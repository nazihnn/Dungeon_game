...
char mdraw[40][100];
DWORD WINAPI mtimes(LPVOID lpParameter);
int limittime = 40;
int limitstep = 120;
clock_t start;
int mcount;
void deleteAllDrawFetid_Air(robot  mrobot, int width, int height)
{
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			if (i == 0 || j == 0 || i == 2 || j == 2)
...
void Draw1(int width, int height, int x, int y)
{
...
void Draw2(int width, int height, int x, int y)
{
	init_pair(1, COLOR_BLACK, COLOR_GREEN);
	init_pair(5, COLOR_BLACK, COLOR_WHITE);
	init_pair(4, COLOR_BLACK, COLOR_RED);
...
void run1()
{
	...
	for (int i = 1, j = 6; i <= 5; i++, j += 3)
	{
		...
		while (flg)
		{
			Draw1(roomwidth[roomid], roomheight[roomid], playpeople.x, playpeople.y);
			attron(COLOR_PAIR(3));
					...
					(playpeople.y == successdoor[1])) && playpeople.nowroom == sroomid)
			{
				...
				{
					rscore += playpeople.Live_Value - playpeople.step + 20 * i;
					mvprintw(4, 80, "Enter to level: %d\n", playpeople.level);
					mvprintw(5, 80, "Score: %d\n", rscore);
					playpeople.level++;
					playpeople.step =0 ;
					mcount = limittime+i*20;
					roomid = MIDDELROOM;
					clear();
					break;
				}
			...
			for (int w = 0; w < j; w++)
			{
				...
				if (playpeople.Live_Value <= 0 || mcount <= 0 || playpeople.step >= limitstep + i * 10)
				{
					...
					mvprintw(11, 40, "To restart the game, press any other key.\n");
					roomid = MIDDELROOM;
					mcount = 0;
					char ch = getch();
					if (ch == 'g')
...
void run2()
{
	...
	for (int i = 1, j = 6; i <= 5; i++, j += 3)
	{
		...
		playpeople.level = i;
		int flg = 1;
		int sroomid = rand() % 4 + 0;
		int sdirect = rand() % 3 + 0;
		if (sroomid == LEFTROOM)
		{
			if (sdirect == 0)
			{
		...
		while (flg)
		{
			Draw2(roomwidth[roomid], roomheight[roomid], playpeople.x, playpeople.y);
			attron(COLOR_PAIR(3));
			mvprintw(playpeople.y, playpeople.x, "*");
			mvprintw(14, 80, "Level:%d ", i);
			mvprintw(13, 80, "Step:%d ", playpeople.step);
	...
	while (1)
	{
		...
		else if (mode == 2)
		{
			...
			if (key == '\r')
			{
				...
				start = clock();
				mcount = limittime;
				HANDLE h1 = ::CreateThread(NULL, 0, mtimes, NULL, 0, NULL);
				run1();
				::CloseHandle(h1);
			}
		}
...
DWORD WINAPI mtimes(LPVOID lpParameter)
{
	...
	while (1)
	{
		if ((clock() - start) == 1000)
		{
			mcount--;
			attron(COLOR_PAIR(2));
			mvprintw(15, 80, "Times:%d", mcount);
			refresh();
			start = clock();
		}
		if (mcount <= 0)
		{
			mvprintw(16, 80, "GAME OVER!");
			getch();
			break;
		}
	}
